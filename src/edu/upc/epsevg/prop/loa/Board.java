...
public class Board extends MouseAdapter {
    ...
    Board(IPlayer player1, IPlayer player2, int timeoutSeconds, Level level) {
    ...
    class Mover extends SwingWorker<Move, Object> {
        ...
        protected void done() {
            try {
                ...
                status.movePiece(m.getFrom(), m.getTo());
            ...
            protected void paintComponent(Graphics g) {
                ...
                for (int i = 0; i < status.getSize(); i++) {
                    ...
                    for (int j = 0; j < status.getSize(); j++) {
                        ...
                        switch (cell) {
                            case PLAYER1:
                                paintQueen(g2d, true, Board.this.getX(j), Board.this.getY(i), midaCasella / 2);
                                break;
                            case PLAYER2:
                                paintQueen(g2d, false, Board.this.getX(j), Board.this.getY(i), midaCasella / 2);
                                break;
                        }
    ...
    public void mouseClicked(MouseEvent me) {
        ...
        if (gameEstatus == UIStates.PLAYING_FROM && !isCurrentPlayerAuto()) {
            ...
            if (selectedQueenPosition != null) {
                ...
                if (status.getPos(selectedQueenPosition) == curPlayer) { // la reina és del mateix color que la que toca jugar
                    ...
                    status.setHighlight(selectedQueenPosition);
                    showCurrentStatus();
                }
            }
            System.out.println(">" + status.toString());
            boardPanel.repaint();
            //endTurn();
        } else if (gameEstatus == UIStates.PLAYING_TO) {
            Point to = convertScreenToBoard(me.getX(), me.getY());
            if (to != null && status.isHighlighted(to)) {
                status.movePiece(selectedQueenPosition, to);
                status.setHighlight(null);
                boardPanel.repaint();
                endTurn();
    ...
    protected void paintQueen(Graphics2D g2, boolean isWhiteQueen, int x, int y, int radius) {
        ...
        //new JLabel(new ImageIcon(new ImageIcon(this.getClass().getResource("/resources/white.png")).getImage().getScaledInstance(30, 30, Image.SCALE_SMOOTH)))  
         
        //int backColor = isWhite ? 0xFFFFFFFF : 0xFF333333;
        int backColor = !isWhiteQueen ? 0xFFFFFFFF : 0xFF111111;
