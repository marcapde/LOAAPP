 ...
 * @author bernat
 */
public class loser2 implements IPlayer, IAuto {
...
//                    }
     ...
     *
     * @param ps tablero resultante de poner una ficha en una determinada
     * columna.
     * @param lastPoint columna en la que hemos puesto ficha para estudiar el
     * tablero.
...
//            int x = AddToHASH(ps,th);
        ...
        for (int i = 0; i < numFitxes; i++) {
            ...
            Point fromAct;
            if (mapa.containsKey(h) && fitxesPos.contains(mapa.get(h).best.getFrom())
                    && i == 0 && mapa.get(h) != null) {
                fromAct = mapa.get(h).best.getFrom();
                fitxesPos.remove(fromAct);
                mv = mapa.get(h).best;
            ...
            if (movPosi.size() > 0) {
                for (int j = 0; 0 < movPosi.size(); j++) {
                    ...
                    if (j == 0 && mapa.containsKey(h)
                            && movPosi.contains(mapa.get(h).best.getTo())) {
                        movAct = mapa.get(h).best.getTo();
